...
- **Shell comes from komyo-kit** (see "Shared kit"): the nav (‹ Menu · mute · komyo ›), the sound
  engine + site-wide mute, the end-screen share row, and PWA registration are all `komyo.*` calls —
...
- `komyo.sound` — shared AudioContext engine + **site-wide mute** (`komyo_muted`). Register a game's
  sounds with `komyo.sound.define({ name: ({tone,noise}) => {…} })`; play with `komyo.sound.play('name')`
  (no-ops when muted/headless). Per-game pattern: `const SND = window.komyo.sound; SND.define({…});`
  then keep every `SND.play('…')` call.
- `komyo.nav({ mute: true })` — injects the top `‹ Menu · 🔊 · komyo ›` bar (Menu reloads; mute toggles sound).
