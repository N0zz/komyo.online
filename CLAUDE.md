...
komyo-kit.js    SHARED game shell — sound engine + mute, top nav, share row, PWA auto-update
komyo-kit.css   shared shell styles (nav buttons, share row)
test.mjs        top-level harness: catalogue + Keep Defender + live-games boot + komyo-kit test
...
- **No external dependencies / assets.** No build, no CDNs/fonts/images — visuals are canvas
  ...
  (`../../komyo-kit.js` + `../../komyo-kit.css`) plus `analytics.js`; these ship with the site and are
...
- **Shell comes from komyo-kit** (see "Shared kit"): the top-left nav (‹ Menu · komyo ›), the
  top-right **sound menu** (SFX + Music + reset-scores), the sound engine, the end-screen share row,
  and PWA registration are all `komyo.*` calls — do NOT re-implement them per game. Put the score/best
  HUD in a **`.komyo-hud`** container (center-top; clears the left nav and right sound menu).
...
## Shared kit (`komyo-kit.js` + `komyo-kit.css`)

Load in `<head>` (NOT `defer`, so `window.komyo` exists before the inline script):
`<link rel="stylesheet" href="../../komyo-kit.css">` then `<script src="../../komyo-kit.js"></script>`.
API on `window.komyo`:

- `komyo.sound` — **SFX** channel: AudioContext engine + per-channel mute & volume
  (`komyo_sfx_muted`/`komyo_sfx_vol`). Register sounds with `komyo.sound.define({ name: ({tone,noise}) => {…} })`;
  play with `komyo.sound.play('name')` (no-ops when muted/headless). Per-game pattern:
  `const SND = window.komyo.sound; SND.define({…});` then keep every `SND.play('…')` call.
- `komyo.music` — **Music** channel (settings + UI only; `komyo_music_muted`/`komyo_music_vol`). A game
  with music keeps its own music engine and does `komyo.music.subscribe(s => applyGain(s.gain))`
  (`s.gain` is 0 when muted). Only asteroids uses this today.
- `komyo.nav({ music, reset, onMenu })` — injects the top-left `‹ Menu · komyo ›` bar **and** the
...
- `komyo.shareRow(el, { slug, title, message })` — builds + wires Native/X/Reddit/Copy into `el`;
...
- `komyo.pwa()` — auto-update SW registration (reload once when a new worker takes control).
- `komyo.resetScores(prefix)` — clears only the localStorage keys starting with `prefix`.
- `komyo.shareUrls(url, msg)` — pure helper the kit test asserts.
- **`.komyo-hud`** (CSS) — the standard **center-top** HUD (translucent pill, wraps; clears the left
  nav and the right sound menu). Games put their score/best/etc. items in a `.komyo-hud` container and
  reserve ~48px (landscape) / ~92px (portrait) top headroom.

Each game's `sw.js` SHELL must include `'../../komyo-kit.js','../../komyo-kit.css'` (offline). The kit
is fully headless-safe (every browser API guarded). **Asteroids is now on the kit too** (launcher +
generated levels; SFX via `komyo.sound`, music gain via `komyo.music.subscribe`, Menu via `onMenu` →
`postMessage`); still mind its generated-trio workflow.

## Adding / changing a game

1. `games/<slug>/index.html` — load komyo-kit in `<head>`; use `komyo.nav`/`komyo.sound`/`komyo.shareRow`/
   `komyo.pwa` for the shell; keep game logic inline with the `__test` hook.
2. `games/<slug>/test.mjs` — dependency-free harness; **preload the kit** (read `../../komyo-kit.js`,
...
- `node test.mjs` — top-level: catalogue + Keep Defender + boots every live game (kit preloaded) +
  a **komyo-kit** test section.
- `node games/<slug>/test.mjs` — each game's own suite (each preloads `../../komyo-kit.js` before the
...
- The inline-script extractor grabs the **last attribute-less `<script>` before `</body>`**, so
  `<script src=...>` (analytics.js, **komyo-kit.js**, gtag) in `<head>` don't confuse it. The kit is
  loaded as a separate sandbox pre-script (the real `komyo-kit.js`). Keep that pattern.
