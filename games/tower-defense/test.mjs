...
function runInline(file) {
  ...
  let bootErr = null;
  try { vm.runInContext(KIT, ctx, { filename: 'komyo-kit.js' }); vm.runInContext(m[1], ctx, { filename: file }); } catch (e) { bootErr = e.stack; }
...
function run() {
  ...
  ok(/id="shareRow"/.test(ov.innerHTML), 'END screen has a komyo-kit share row mount');
  ...
  const su = g2.win.komyo.shareUrls('https://komyo.online/games/tower-defense/', 'I survived 3 waves in Keep Defender 🏰');
