...
function runGame(file, dims) {
  ...
  let bootErr = null;
  try { vm.runInContext(KIT, ctx, { filename: 'komyo-kit.js' }); vm.runInContext(code, ctx, { filename: file }); }
...
const g4 = (() => {
  ...
  sandbox.globalThis = sandbox;
  const ctx = vm.createContext(sandbox);
  try { vm.runInContext(KIT, ctx, { filename: 'komyo-kit.js' }); vm.runInContext(m[1], ctx, { filename: 'index.html' }); } catch (e) {}
